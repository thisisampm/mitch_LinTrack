function tuning_curve_matrix = all_cell_tuning(session_files, cell_regist_mtx, reference_session)
...
tuning_curve_matrix = nan(size(cell_regist_mtx,1), num_spatial_bins, num_sessions);
for isesh = 1:num_sessions
    ...
    % common cells only
    tuning_curve_matrix(cell_regist_mtx(:,isesh)>0,:,isesh) = unsorted_session_tuning_curves(cell_regist_mtx(cell_regist_mtx(:,isesh)>0,isesh),:);
...
tuning_curve_matrix = tuning_curve_matrix(~isnan(tuning_curve_matrix(:,1,reference_session)),:,:);

% sort by peak
[~,sort_idx] = sort_rows_by_peak(tuning_curve_matrix(:,:,reference_session));
